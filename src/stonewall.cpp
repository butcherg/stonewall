...
int main(int argc, char **argv) {
	...
	for (unsigned i=1; i<argc; i++) {
		...
		else if (string(argv[i]).find("texturetest") != string::npos) { 
			texturetest = atoi(val(argv[i]).c_str());
		}
	...
	if (destimage.size() == 0) { // spit out OpenSCAD polygon points
		...
		for (const auto& contour : culledcontours) {
			...
			Rect r = boundingRect(contour);
			string wx = to_string(r.width);
			string wy = to_string(r.height);
			string bx = to_string(dist(gen));
			string by = to_string(dist(gen));
			string cmdstring = "noisetool " + noisefile + " destfile=texture.txt bounds="+bx+","+by+","+wx+","+wy+ " destsize=" + wx + "," + wy;
			cout << cmdstring << endl;
			...
			//manifold::ExportMesh("stonetest/tex"+to_string(c)+".stl", stonetext.GetMeshGL(), {});
			...
			//save the stone mesh:
			manifold::ExportMesh(to_string(c)+".3mf", stone.GetMeshGL(), {});
